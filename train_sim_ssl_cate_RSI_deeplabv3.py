...
# DATA_DIRECTORY = '/home/yb/dataset/IAILD2MassBuilding'
# DATA_LIST_PATH = './dataset/Inria_list/train.txt'
# IGNORE_LABEL = 255
# INPUT_SIZE = '512,512'
# DATA_DIRECTORY_TARGET = '/home/yb/dataset/Massachusetts_Building_dataset'
# DATA_LIST_PATH_TARGET = './dataset/MassBuilding_list/train.txt'
# DATA_LIST_PATH_TARGET_TEST = './dataset/MassBuilding_list/val.txt'
# INPUT_SIZE_TARGET = '512,512'
# LEARNING_RATE = 2.5e-4
# MOMENTUM = 0.9
# NUM_CLASSES = 2
# NUM_STEPS = 220000
# NUM_STEPS_STOP = 220000  # early stopping
# NUM_PROTOTYPE = 2#50
# POWER = 0.9
# RANDOM_SEED = 1234
# RESTORE_FROM = './snapshots_ssl/Inria2MassBuilding_cate+ins_deeplabv3+/Inria2MassBuilding_190000.pth'
# #RESTORE_FROM = 'https://download.pytorch.org/models/resnet101-5d3b4d8f.pth'
# SAVE_NUM_IMAGES = 2
# SAVE_PRED_EVERY = 5000
# SNAPSHOT_DIR = './snapshots_ssl/Inria2MassBuilding_cate+ins_deeplabv3+'
# WEIGHT_DECAY = 0.0005
# LOG_DIR = './log'
# SAVE_PATH = './result/Inria2MassBuilding_deeplabv3+'
...
#
# LAMBDA_ADV_CLS = 0.01
# LAMBDA_ADV_INS = 0.0001#0.001
# LAMBDA_ADV_CATE = 0.0001#0.001
# BETA = 1.0
#
# ### cate2ins
...
#
#     # init data loader
#     if args.data_dir.split('/')[-1] == 'IAILD2MassBuilding':
#         trainset = InriaDataSet(args.data_dir, args.data_list,
#                                        max_iters=args.num_steps * args.iter_size * args.batch_size,
#                                        crop_size=input_size,
#                                        scale=args.random_scale, mirror=args.random_mirror, mean=IMG_MEAN)
#     elif args.data_dir.split('/')[-1] == 'IAILD':
#         trainset = InriaDataSet(args.data_dir, args.data_list,
#                                        max_iters=args.num_steps * args.iter_size * args.batch_size,
#                                        crop_size=input_size,
#                                        scale=args.random_scale, mirror=args.random_mirror, mean=IMG_MEAN)
...
#
#     targetloader = data.DataLoader(MassBuildingDataSet(args.data_dir_target, args.data_list_target,
#                                                 max_iters=args.num_steps * args.iter_size * args.batch_size,
...
#     test_interp = nn.Upsample(size=(300, 300), mode='bilinear', align_corners=True)
#
...
#     src_cls_features = torch.zeros([len(BG_LABEL), num_prototype, 256], dtype=torch.float32).to(device)
#     src_cls_ptr = np.zeros(len(BG_LABEL), dtype=np.uint64)
#     src_ins_features = torch.zeros([len(FG_LABEL), num_ins, 256], dtype=torch.float32).to(device)
#     src_ins_ptr = np.zeros(len(FG_LABEL), dtype=np.uint64)
#
#     src_all_features = torch.zeros([len(ALL_LABEL), num_ins, 256], dtype=torch.float32).to(device)
...
#
#             loss_seg_source = seg_loss(pred, labels)
#
#             right_label = F.interpolate(labels.unsqueeze(0).float(), (pred_idx.size(1), pred_idx.size(2)),
...
#                 #     continue
#                 src_cls_features[ii, int(src_cls_ptr[ii] % num_prototype), :] = torch.squeeze(feature).clone().detach()[1:]
#                 src_cls_ptr[ii] += 1
#
...
#                     # if cls_idx != torch.argmax(torch.squeeze(model.layer6(feature.half()).float())).item():
#                     #     continue
#                     src_ins_features[ii, int(src_ins_ptr[ii] % num_ins), :] = torch.squeeze(feature).clone().detach()[1:]
#                     src_ins_ptr[ii] += 1
#
#             pred = interp(pred)
...
#             #print('pred_target_idx:',pred_target_idx)
#
...
#
#             # ISIA loss
#             loss_cate_same = torch.zeros(1).to(device)
#             loss_cate_diff = torch.zeros(1).to(device)
#             # for i in range(0, NUM_CLASSES):
#             #     indices = torch.tensor(i).to(device)
#             #     # source_feature = torch.index_select(pred_softmax, 1, indices)
#             #     # target_feature = torch.index_select(pred_target_softmax, 1, indices)
#             #     source_feature = pred_softmax[:,i:i+1,:,:]
#             #     target_feature = pred_target_softmax[:,i:i+1,:,:]
#             #     #print('source_feature{}:'.format(i),source_feature.size(),'target_feature{}:'.format(i),target_feature.size())
#             #     loss_cate_same += torch.sum(L1_loss(source_feature, target_feature)) / 512.
#             # #
#             # for i in range(0, NUM_CLASSES):
#             #     indices = torch.tensor(i).to(device)
#             #     source_feature = torch.index_select(pred_softmax, 1, indices)
...
#             #         if i == j:
#             #             continue
#             #         indices_T = torch.tensor(j).to(device)
#             #         target_feature = torch.index_select(pred_target_softmax, 1, indices_T)
#             #         loss_cate_diff += torch.sum(
#             #             0.5 + 0.5 * torch.cosine_similarity(source_feature, target_feature, dim=1)) \
#             #                           / (source_feature.size()[2] * source_feature.size()[3]) / 512.
#
#
#             if i_iter > 0:
#                 for ii in range(len(BG_LABEL)):
...
#                     ext_feature = feature.squeeze().expand(num_prototype, 256)
#                     #loss_cls += CATEGORY_BG_COFF[ii] * torch.min(
#                     #    torch.sum(L1_loss(ext_feature, src_cls_features[ii, :, :]), dim=1) / 2048.)
#                     loss_cls += CATEGORY_BG_COFF[ii] * torch.min(
#                         torch.sum(L1_loss(ext_feature, src_cls_features[ii, :, :]), dim=1) / 256.)
#
...
#                         feature = feature.squeeze().expand(num_ins, 256)
#                         # loss_ins += torch.min(
#                         #     torch.sum(L1_loss(feature, src_ins_features[ii, :, :]), dim=1) / 2048.) / min(100,
#                         #                                                                                   len(sortmax))
#                         loss_ins += torch.min(
#                             torch.sum(L1_loss(feature, src_ins_features[ii, :, :]), dim=1) / 256.) / min(100, len(sortmax))
#                         #loss_ins += BETA* CATEGORY_INS_COFF[i] * torch.min(torch.sum(L1_loss(feature, src_ins_features[ii,:,:]),dim=1) / 256.) / min(100, len(sortmax))
#
#
#                 # for ii in range(len(BG_LABEL)):
...
#                 #             loss_cate_dist += torch.sum(
#                 #                 0.5 + 0.5 * torch.cosine_similarity(ext_feature, feature_FG, dim=1)) / 2048. / min(100,len(sortmax))
#
...
#         #         i_iter, args.num_steps, loss_seg_value, loss_seg_trg.item(), loss_adv_target_value, loss_D_value,
#         #         loss_cls.item(), loss_ins.item(), loss_cate_dist.item()))
#         print(
#             'iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}, loss_seg_trg = {3:.3f}, loss_D = {4:.3f}, loss_cls={5:.3f}, loss_ins={6:.3f} loss_cate_same = {7:.3f}, loss_cate_diff = {8:.3f}'.format(
#                 i_iter, args.num_steps, loss_seg_value, loss_adv_target_value, loss_D_value, loss_cls.item(), loss_ins.item(), loss_cate_same.item(), loss_cate_diff.item()))
#
#         if i_iter >= args.num_steps_stop - 1:
#             print('save model ...')
#             torch.save(model.state_dict(),
#                        osp.join(args.snapshot_dir, 'Inria2MassBuilding_' + str(args.num_steps_stop) + '.pth'))
#             torch.save(model_D.state_dict(),
#                        osp.join(args.snapshot_dir, 'Inria2MassBuilding_' + str(args.num_steps_stop) + '_D.pth'))
#             break
#
...
#             testloader = data.DataLoader(MassBuildingDataSet(args.data_dir_target, args.data_list_target_test,
...
#             mIoUs = compute_mIoU_RSI(osp.join(args.data_dir_target, 'SegmentationClass'), args.save,
#                                      'dataset/MassBuilding_list')
...
#             torch.save(model.state_dict(), osp.join(args.snapshot_dir, 'Inria2MassBuilding_' + str(i_iter) + '.pth'))
#             torch.save(model_D.state_dict(),
#                        osp.join(args.snapshot_dir, 'Inria2MassBuilding_' + str(i_iter) + '_D.pth'))
#             model.train()
#
#     if args.tensorboard:
#         writer.close()
...
NUM_WORKERS = 4
#DATA_DIRECTORY = '/home/yb/dataset/Massachusetts_Building_dataset'
DATA_DIRECTORY = '/home/yb/dataset/DeepGlobe2MassRoad'
DATA_LIST_PATH = './dataset/DeepGlobe_list/train.txt'
IGNORE_LABEL = 255
INPUT_SIZE = '512,512'
DATA_DIRECTORY_TARGET = '/home/yb/dataset/Massachusetts_Road_dataset'
...
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM = './snapshots/DeepGlobe2MassRoad_FL_deeplabv3+/Best.pth'
#RESTORE_FROM = 'https://download.pytorch.org/models/resnet101-5d3b4d8f.pth'
SAVE_NUM_IMAGES = 2
SAVE_PRED_EVERY = 5000
SNAPSHOT_DIR = './snapshots_ssl/DeepGlobe2MassRoad_FL_deeplabv3+'
WEIGHT_DECAY = 0.0005
LOG_DIR = './log'
SAVE_PATH = './result/DeepGlobe2MassRoad_FL_deeplabv3+'
SSL_TARGET_DIR = './target_ssl_gt_MassRoad' # Inria trainset pseudo GT
...
def main():
    ...
    # init data loader
    if args.data_dir.split('/')[-1] == 'DeepGlobe2MassRoad':
        trainset = DeepGlobeDataSet(args.data_dir, args.data_list,
                                       max_iters=args.num_steps * args.iter_size * args.batch_size,
                                       crop_size=input_size,
                                       scale=args.random_scale, mirror=args.random_mirror, mean=IMG_MEAN)
    ...
    for i_iter in range(Iter, args.num_steps):
        ...
        if i_iter >= args.num_steps_stop - 1:
            print('save model ...')
            torch.save(model.state_dict(),
                       osp.join(args.snapshot_dir, 'DeepGlobe2MassRoad_' + str(args.num_steps_stop) + '.pth'))
            torch.save(model_D.state_dict(),
                       osp.join(args.snapshot_dir, 'DeepGlobe2MassRoad_' + str(args.num_steps_stop) + '_D.pth'))
            break
        ...
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            ...
            torch.save(model_D.state_dict(),
                       osp.join(args.snapshot_dir, 'DeepGlobe2MassRoad_' + str(i_iter) + '_D.pth'))
            model.train()
