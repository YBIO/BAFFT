...
RESTORE_FROM = './snapshots_ssl/GTA2Cityscapes-cate2ins-4-new/GTA5_95000.pth'
SAVE_NUM_IMAGES = 2
SAVE_PRED_EVERY = 10#5000
SNAPSHOT_DIR = './snapshots_ssl/GTA2Cityscapes-temp'
WEIGHT_DECAY = 0.0005
LOG_DIR = './log'
SAVE_PATH = './result/cityscapes-temp'
...
LAMBDA_ADV_INS = 0.02#0.001
LAMBDA_ADV_CATE = 0.1#0.001
BETA = 10
...
def plot_embedding(data, label, title):
    ...
    fig = plt.figure(dpi=300,figsize=(14.0,8.0))
    for i in range(data.shape[0]):
        plt.text(data[i,0],data[i,1],str(label[i]),
                color = plt.cm.Set1(label[i]/34.0),
...
def plot_embedding_3d(data, label, title):
    ...
    for i in range(data.shape[0]):
        plt.text(data[i,0],data[i,1],data[i,2],str(label[i]),
        ...
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            ...
            for index, batch in enumerate(testloader):
                ...
                with torch.no_grad():
                    ...
                    print('output1:',output1.size(),output2.size())
                print('---generating tSNE:')
                tsne = TSNE(n_components=2, init='pca', random_state=0)
                tSNE_feature = output1.squeeze(0).data.cpu().numpy() # (2048,65,129)
                print('before_feature:',tSNE_feature.shape)
                tSNE_feature=tSNE_feature.transpose(1,2,0) #(65, 129, 2048)
                print('tSNE_feature-swap:',tSNE_feature.shape)
                # second resize
                tSNE_feature = np.resize(tSNE_feature,(65,129,2048))
                tSNE_feature = np.reshape(tSNE_feature,(-1,2048)) #(8385, 2048)
                print('tSNE_feature-reshape:',tSNE_feature.shape)
                # read GT 
                import cv2
                label_img = cv2.imread('./data/Cityscapes/gtFine/val/'+str(name[0]).split('_')[0]+'_'+str(name[0]).split('_')[1]+'_'+str(name[0]).split('_')[2]+'_gtFine_labelIds.png',0)
                print('label_img:',label_img.shape)
                label_img = cv2.resize(label_img, (65,129))

                tSNE_result = tsne.fit_transform(tSNE_feature)
                
                #second resize
                tSNE_label = np.resize(label_img,(65,129))
                tSNE_label = np.reshape(tSNE_label,(-1))
                print('tSNE:',tSNE_feature.shape, tSNE_label.shape)
                fig = plot_embedding(tSNE_result, tSNE_label, '')
                plt.savefig('./TSNE_figs14vs8/{}.png'.format(str(name[0]).split('/')[1].split('_')[0]+'_'+str(name[0]).split('_')[1]+'_'+str(name[0]).split('_')[2]+'_gtFine_labelIds'))
