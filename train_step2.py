...
SNAPSHOT_DIR = './snapshots_ssl/GTA2Cityscapes-cate2ins-selfadapt'
WEIGHT_DECAY = 0.0005
LOG_DIR = './log'
            ...
            if i_iter > 0:
                ...
                for ii in range(len(FG_LABEL)):
                    ...
                    for i in range(min(10, len(sortmax))):
                        ...
                        print('len(sortmax){}:'.format(i),len(sortmax))
                        print('target_feature{}:'.format(i),len(target_features))
                        loss_cate2ins += CATEGORY_COFF[i]*torch.min(torch.sum(L1_loss(feature, src_ins_features[ii,:,:]),dim=1) / 2048.) / min(10, len(sortmax))
                    


                # enlarge gap between categories
                BG_Target_feature_list = []
                for ii in range(len(BG_LABEL)):
                    cls_idx = BG_LABEL[ii]
                    if src_cls_ptr[ii] / num_prototype <= 1:
                        continue
                    ...
                    if cls_idx != torch.argmax(torch.squeeze(model.layer6(feature.half()).float())).item():
                        continue
                    ext_feature = feature.squeeze().expand(num_prototype, 2048)
                    BG_Target_feature_list.append(ext_feature)
                for ii in range(len(BG_Target_feature_list)):
                    if ii < len(BG_Target_feature_list):
                        loss_cate_dist_BG += torch.sum(0.5+0.5*torch.cosine_similarity(BG_Target_feature_list[ii], BG_Target_feature_list[ii+1], dim=1)) / 2048. 
                    else:
                        loss_cate_dist_BG += torch.sum(0.5+0.5*torch.cosine_similarity(BG_Target_feature_list[ii], BG_Target_feature_list[0], dim=1)) / 2048. 

                FG_Target_feature_list = []
                seg_ins = seg_label(pred_target_idx.squeeze())
                for ii in range(len(FG_LABEL)):
                    cls_idx = FG_LABEL[ii]
                    if src_ins_ptr[ii] / num_ins <= 1:
                    ...
                    for i in range(min(10, len(sortmax))):
                        ...
                        feature = global_avg_pool(trg_feature, mask.float())
                        feature = feature.squeeze().expand(num_ins, 2048)
                        FG_Target_feature_list.append(feature)
                        loss_cate_dist_FG += CATEGORY_INS_COFF[i] * torch.min(torch.sum(L1_loss(feature, src_ins_features[ii,:,:]),dim=1) / 2048.) / min(10, len(sortmax))
                ...
                for ii in range(len(BG_LABEL)):
                    ...
                    for jj in range(len(FG_LABEL)):
                        ...
                        for i in range(min(10, len(sortmax))):
                            mask = segmask==(sortmax[i]+1)
                            feature_FG = global_avg_pool(trg_feature, mask.float())
                            feature_FG = feature_FG.squeeze().expand(num_ins, 2048)
