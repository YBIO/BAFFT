...
SNAPSHOT_DIR = './snapshots/SYN2Cityscapes/'
WEIGHT_DECAY = 0.0005
LOG_DIR = './log'
SAVE_PATH = './result/SYN2Cityscapes'
...
def main():
    ...
    for i_iter in range(Iter, args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            if i_iter > 0:
                ...
                for ii in range(len(FG_LABEL)):
                    ...
                    for i in range(min(10, len(sortmax))):
                        ...
                        loss_ins += CATEGORY_INS_COFF[ii] *torch.min(torch.sum(L1_loss(feature, src_ins_features[ii,:,:]),dim=1) / 2048.) / min(10, len(sortmax))
            ...
            loss = args.lambda_adv_target * loss_adv_target + args.lambda_adv_cls * loss_cls + args.lambda_adv_ins * loss_ins + args.lambda_adv_cate * loss_cate_dist
            loss = loss / args.iter_size
            amp_backward(loss, optimizer)
        ...
        print(
        'iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}, loss_adv = {3:.3f} loss_D = {4:.3f} loss_cls = {5:.3f} loss_ins = {6:.3f} loss_cate_dist = {7:.3f}'.format(
            i_iter, args.num_steps, loss_seg_value, loss_adv_target_value, loss_D_value, loss_cls.item(), loss_ins.item(), loss_cate_dist.item()))
